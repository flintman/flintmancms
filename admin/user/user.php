    ...
    if (!$_POST['submit']) {
        ...
        If (isset($_POST['submit'])) {
            ...
            if ($password != "" && $password = $password2) {
                ...
                $sql = sprintf("UPDATE " . DB_PREFIX . "_profile SET password=%n WHERE id=%n",
                                quote_smart($password), quote_smart($id));
                $result = mysql_query($sql)
                        or $errorMsg = "ERROR: " . mysql_error() . " @ Line " . __LINE__ . " Of " . __FILE__;
            }
            $sql = sprintf("UPDATE " . DB_PREFIX . "_profile SET
                username=%s,email=%s,permissions=%s WHERE id=%s",
                            quote_smart($username), quote_smart($email), quote_smart($groups), quote_smart($id));
            $result = mysql_query($sql) or $errorMsg = "ERROR: " . mysql_error() .
