...
// Slightly Modified by Triste of Intrigue (www.intrigue-guild.org)
// MOD_TR - #1 - Modification to allow the specification of attributes for the TH of HTML outputs.
...
// MOD_TR - #1 - INS 2 Lines
	* @param string $strColumnAttribute This text is added 'as is' to the TH field at the top of the report.
	*	Intended use is for width etc
	* @return void
	* @access public
	*/ 
	// MOD_TR - #1 - EDIT 1 Line (original, replacement)
	//function addOutputColumn($strFieldName, $strColumnTitle, $strFormat = null, $strAlign = null, $intLine = null, $strFieldLink = null, $strLinkDesc = null)
	function addOutputColumn($strFieldName, $strColumnTitle, $strFormat = null, $strAlign = null,  $intLine = null, $strFieldLink = null, $strLinkDesc = null, $strColumnAttribute = null)
	{
		...
		// MOD_TR - #1 - INS 1 Line
		$arrColumn['attribute'] = $strColumnAttribute;
	...
	*/ 
	...
	function getListFromArray($arrRecords, $blnDoForDownload = false, $strOutputMethod = null)
	{
		...
		if ($intTotalRecords == 0) {
			...
			$strList .= $this->_makeListTitles('<th>None</th>', $strOutputMethod);
		...
		if ($this->_arrSortInfo['blnAllowSort'] && $this->_arrSortInfo['strSortField'] != '') {
			...
			foreach ($arrRecords as $curRecord) {
				// dshax lower strings before sorting.
				$sortarray[] = strtolower($curRecord[$this->_arrSortInfo['strSortField']]);
			}
		...
		if (!($blnDoMultiLineList) && $this->_arrColumnInfo['blnShowColumnNames'] && ($strOutputMethod != 'xml')) {
			...
			foreach ($arrColumnsToUse as $arrCol) {
				...
				if ($arrCol['align'] != '') {
					...
					$strRow .= '<th align="' . $arrCol['align'] . '" ' . $arrCol['attribute'] . '>';
				}
				else { $strRow .= '<th>'; }
				...
				if ($this->_arrSortInfo['blnAllowSort'] && !($blnDoForDownload)) {
					if($strColTitle == 'Admin' || $strColTitle == 'Signup')
					{
						$strColTitle = "";
					}
					elseif($strColTitle == 'Race / Class')
					{
						$strColTitle = 'Race / Class';
					}
					elseif(substr($strColTitle,0,2) != '<i')
					{
						if ($arrCol['name'] == $this->_arrSortInfo['strSortField'] && !($this->_arrSortInfo['blnSortDescending'])) {
							...
							if ($this->_arrSortInfo['blnUsePathParams'] == 1) {
								$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . '/SortDescending-1" onMouseover="ddrivetip(\'Click to sort by ' . htmlspecialchars(strip_tags($strColTitle)) . ' \')"; onMouseout="hideddrivetip()">' . $strColTitle . '</a> ';
							}
							else {
								$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . '&SortDescending=1' . $this->_arrSortInfo['strSortAnchor'] . '" onMouseover="ddrivetip(\'Click to sort by ' . htmlspecialchars(strip_tags($strColTitle)) . ' \')"; onMouseout="hideddrivetip()">' . $strColTitle . '</a>';
							}
						}
						else {
							// Give the option to sort (ascending) by this field
							$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . $this->_arrSortInfo['strSortAnchor'] . '" onMouseover="ddrivetip(\'Click to sort by ' . htmlspecialchars(strip_tags($strColTitle)) . '\')"; onMouseout="hideddrivetip()">' . $strColTitle . '</a> ';
						}
					}
					else
					{
						if ($arrCol['name'] == $this->_arrSortInfo['strSortField'] && !($this->_arrSortInfo['blnSortDescending'])) {
							// Give the option to sort descending
							if ($this->_arrSortInfo['blnUsePathParams'] == 1) {
								$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . '/SortDescending-1">' . $strColTitle . '</a>';
							}
							else {
								$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . '&SortDescending=1' . $this->_arrSortInfo['strSortAnchor'] . '">' . $strColTitle . '</a>';
							}
						}
						else {
							// Give the option to sort (ascending) by this field
							$strColTitle = '<a href="' . $this->_arrSortInfo['strSortURL'] . urlencode($arrCol['name']) . $this->_arrSortInfo['strSortAnchor'] . '">' . $strColTitle . '</a>';
						}
					}
				}
		...
		for ($iRow = $intFirstRecord; $iRow < $intLastRecord; $iRow++) {
			...
			$strList .= $this->_makeListRow($strRow, $strOutputMethod, $strHyperlink);
		}
	...
	* @access private
	*/
	function _makeListRow($strRow, $strOutputMethod = 'html', $strHyperlink = '')
	{
		if ($strOutputMethod == 'tab' || $strOutputMethod == 'csv' || $strOutputMethod == 'xml') {
			...
			if ($this->_arrLinkInfo['blnAllowLink']) {
				...
				if ($this->_arrRowInfo['strHoverRowClassName'] != '') {
					$strHoverScript =
					'onmouseover="this.className=\'' . $this->_arrRowInfo['strHoverRowClassName'] . '\';" '.
					'onmouseout="this.className=\'' . $strClassName . '\';" '.
					'onclick="self.location=\'' . ltrim($strHyperlink, '"') . '\';" ';
				}
				else { $strHoverScript = ''; }
	...
	*/
	function _makePageNavigators($intTotalRecords, $intNumListColumns)
	{
		...
		if ($intStartRecord > 0) {
			...
			$strLink = '<a href="%27%20.%20$this-%3E_arrPagingInfo%5B%27strPagingURL%27%5D%20.%20$intBase%20.%20$strSortOptions%20.%20%27">';
	...
	* @param boolean $blnOpenQuotes True/False for whether or not to open the quotes of the link,
	*	useful in case additional parameters must be added to the link by some other method
	* @return string
	* @access private
	*/
	function _buildLinkURLFromRecord($arrRecord, $blnCloseQuotes = true, $blnOpenQuotes = true)
	{
		$strURL = '';

		if ($blnOpenQuotes) { $strURL .= '"'; }
	...
	function _formatListValue($strValue, $strFormat)
	{
		switch ($strFormat) {
			...
			case 'money' :
				...
				break;
			case 'dollars' :
				$db_raidlValue = doubleval($strValue);
				if ($db_raidlValue < 0) {
					$strValue = number_format(-1 * $db_raidlValue, 2);
					$strValue = '($' . $strValue . ')';
				}
				else {
					$strValue = number_format($db_raidlValue, 2);
					$strValue = '$' . $strValue;
				}
				break;
			case 'decimal' :
